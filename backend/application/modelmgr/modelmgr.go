...
import (
	...
	configModel "github.com/coze-dev/coze-studio/backend/api/model/admin/config"
...
func (m *ModelmgrApplicationService) GetModelList(ctx context.Context, _ *developer_api.GetTypeListRequest) (
	...
	mList, err := config.ModelConf().GetModelList(ctx)
	if err != nil {
		return nil, err
	}

	logs.Debug(ctx, "GetModelList, mList: %v", conv.DebugJsonToStr(mList))

	locale := i18n.GetLocale(ctx)
	modelList, err := slices.TransformWithErrorCheck(mList, func(mm *modelmgr.Model) (*developer_api.Model, error) {
...
func modelDo2To(m *modelmgr.Model, locale i18n.Locale) (*developer_api.Model, error) {
	...
	return &developer_api.Model{
		...
		IsOffline:      model.Status != configModel.ModelStatus_StatusInUse,
